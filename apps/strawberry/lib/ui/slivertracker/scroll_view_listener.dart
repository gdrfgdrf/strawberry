...
class ScrollViewListener extends StatefulWidget {
  ...
  const ScrollViewListener({super.key, required this.child, this.onScrollEnd, this.onScrollUpdate});
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      if (mounted) { // 当元素处于PageView中时，可能执行initState但是实际不发生渲染，并且context为null
        // 确定当前ScrollableState
        BuildContext? childEle = context;
        while (childEle != null) {
          if (childEle!.widget is Scrollable) {
            _scrollableState = (childEle as StatefulElement).state as ScrollableState?;
            break;
          } else {
        ...
        Stream<ScrollNotification>? ancestorScrollViewListener = ScrollViewListener
            .of(context);
        if (ancestorScrollViewListener != null) {
          sb = ancestorScrollViewListener.listen((
